...
def main(separator='\t'):
    ...
    #   group - c_vol, c_class, c_dist, c_proxi items
    for c_point, group in groupby(data, itemgetter(0)):
        ...
        for c_point, c_contents in group:
            c_vol, c_class, c_dist, c_proxi = c_contents.rstrip().split('\t')
            if(c_class == "0"):
                ...
                loop_p = loop_p + 1
            avg_proxi = avg_proxi + np.float(c_proxi)
            loop_t = loop_t + 1
        avg_dist = (avg_p_dist/loop_p) - (avg_n_dist/loop_n)
        avg_proxi = avg_proxi/loop_t
        print "%s%s\t%0.4f\t%0.4f" % (c_point, separator, np.round(avg_dist,4), np.round(avg_proxi,4))
